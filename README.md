...
- **API:** YooKassa v3 (`api.yookassa.ru`)
- **Бэкенд:** `payment.js` на порту 3457 (PM2, процесс `payment-yookassa`)
- **Виджет:** YooMoneyCheckoutWidget с `https://yookassa.ru/checkout-widget/v1/checkout-widget.js`
- **Прокси:** Apache ProxyPass `/create-payment` → localhost:3457
- **Ключи:** `/var/www/eddytester.com/.env` (shopId + secretKey)
- **После оплаты:** редирект на `https://eddytester.com/api`
- **Тестовая страница:** `https://eddytester.com/test-yookassa.html`
- **Важно:** `test: true` в API работает только с отдельным тестовым магазином (создаётся в ЛК ЮKassa)

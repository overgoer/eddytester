...
### 8. Conversion optimization (ветка `feat/conversion-optimization`, тестируется)

- ✅ Click-трекинг: dataLayer.push({event: "begin_checkout", ecommerce: {...}}) на кнопках покупки
- ✅ Блок гарантии возврата: "Гарантия возврата: если не понравится — вернём деньги в течение 14 дней" под кнопкой в блоке стоимости
- ⬜ WebP-картинки: отложено (блоки поехали на тесте, нужна проверка MIME-типов на сервере)
